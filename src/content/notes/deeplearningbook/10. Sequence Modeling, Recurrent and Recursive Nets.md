...
$$
\mathbfit{h}^{(t)}=f(\mathbfit{h}^{(t-1)},\mathbfit{x}^{(t)};\boldsymbol{\theta})
$$
...
The unfolded representation has the advantage of allowing us to represent the function underlying $\mathbfit{h}^{(t)}$
$$
\mathbfit{h}^{(t)}=g^{(t)}(\mathbfit{x}^{(t)},\mathbfit{x}^{(t-1)},\dots ,\mathbfit{x}^{(1)})
$$
...
There are RNNs that similarly produce an output at each time step, but have recurrent connections only from $\mathbfit{o}^{(t-1)}\to \mathbfit{h}^{(t)}$.
...
Now, returning to the basic RNN structure, let's consider how we would train the network. Given some output sequence $\mathbfit{y}$ and some input sequence $\mathbfit{x}^{(1)},\dots,\mathbfit{x}^{(t)}$, the negative log-likelihood is
$$
-\sum_{t}\log p_{\text{model}}(y^{(t)}\mid \{ \mathbfit{x}^{(1)},\dots ,\mathbfit{x}^{(t)} \})
$$
...
Recall the RNN structure with only recurrent connection from $\mathbfit{o}^{(t-1)}\to \mathbfit{h}^{(t)}$. It is a *less powerful* model because it lacks hidden-to-hidden recurrence, and thus relies on the output units to capture all information about the past, which is generally less effective since the model is trained to match the output to the training set targets.

However, this structure has a key advantage—the time steps are all *decoupled* for back-propagation! The idea is that the training set already provides the ideal value of $\mathbfit{o}^{(t-1)}$; therefore, the gradient computation can be parallelized, with each time step being handled independently by using the training set's ideal $\mathbfit{o}^{(t-1)}$ to train $\mathbfit{h}^{(t)}$.

This method can be extended to the *forward propagation* process too, in what is known as **teacher forcing**. In essence, instead of feeding $\mathbfit{o}^{(t-1)}$ through the recurrent connection as input for $\mathbfit{h}^{(t)}$, we use $\mathbfit{y}^{(t-1)}$.
...
$$
\log p(\mathbfit{y}^{(1)},\mathbfit{y}^{(2)}\mid \mathbfit{x}^{(1)},\mathbfit{x}^{(2)})
$$
Since $\mathbfit{y}^{(2)}$ is dependent on $\mathbfit{y}^{(1)}$, this is equivalent to
$$
\log p(\mathbfit{y}^{(2)}\mid \mathbfit{y}^{(1)},\mathbfit{x}^{(1)},\mathbfit{x}^{(2)}) + \log p(\mathbfit{y}^{(1)}\mid \mathbfit{x}^{(1)},\mathbfit{x}^{(2)})
$$
...
> [!info]+
> Teacher forcing can be used for models with $\mathbfit{h}^{(t-1)}\to \mathbfit{h}^{(t)}$ connections, provided there are $\mathbfit{o}^{(t-1)}\to \mathbfit{h}^{(t)}$ connections. However, it loses the efficiency benefits, as any $\mathbfit{h}^{(t-1)}\to \mathbfit{h}^{(t)}$ connections require BPTT to train.
...
RNNs possess an *efficient parameterization* of the unrolled graphical model, with their parameter sharing. However, this can cause difficulties in parameter optimization; in particular, it expresses a strong prior over the model that assumes the conditional probability distribution $p(\mathbfit{v}_{t+1}\mid \mathbfit{v}_{t})$, where $\mathbfit{v}_{t}$ represents all variables (hidden units, output units, etc.) at time $t$, is **stationary**, i.e. it  does not depend on $t$.
...
Sometimes, rather than taking a sequence of vectors $\mathbfit{x}^{(t)}$ as input, the RNN receives a single, fixed-length vector $\mathbfit{x}$. Then, it's common to do one of a few things.

- Add $\mathbfit{x}$ as an extra input to every time step.
- Use it as the initial state $\mathbfit{h}^{(0)}$.
- Both!
...
Formally, if $\mathbfit{W}$ is the weight matrix describing the hidden-to-hidden recurrence relation, and it has eigendecomposition $\mathbfit{W}=\mathbfit{Q}\boldsymbol{\Lambda}\mathbfit{Q}^{\top}$, then
$$
\mathbfit{h}^{(t_{2})}=\mathbfit{Q}^{\top}\boldsymbol{\Lambda}^{t_{2}-t_{1}}\mathbfit{Q}h^{(t_{1})}
$$
Any eigenvalues with $\lvert \lambda \rvert>1$ will explode, while those with $\lvert \lambda \rvert<1$ will vanish as $t_{2}-t_{1}\to \infty$. Eventually, any component of $\mathbfit{h}^{(t_{2})}$ not aligned with the largest eigenvector will become irrelevant.
...
Considering a linear network, i.e. $\mathbfit{h}^{(t+1)}={\mathbfit{h}^{(t)}}^{\top}\mathbfit{W}$, we say that a linear map $\mathbfit{W}$ is **contractive** if it shrinks $\lVert \mathbfit{h} \rVert_{2}$. This occurs if $\mathbfit{J}^{(t)}<1$, and actually may help the network *forget* unnecessary information; which is a must considering the finite precision of the state vector.
...
$$
where $\mathbfit{x}^{(t)}$ is the current input vector and $\mathbfit{h}^{9t}$ is the current hidden layer vector. Also, $\mathbfit{b}^{f},\mathbfit{U}^{f},\mathbfit{W}^{f}$ are the biases, input weights, and recurrent weights for the forget gates.
...
**Gradient clipping** is a family of methods designed to reduce the impact of such cliff updates. For instance, one may "clip" the norm $\lVert \mathbfit{g} \rVert$ of the gradient $\mathbfit{g}$ before the parameter update
$$
\mathbfit{g}\leftarrow \frac{\mathbfit{g}v}{\lVert \mathbfit{g} \rVert },\text{ if }\lVert \mathbfit{g} \rVert >v
$$
for some constant norm threshold $v$. Another method clips each element of the gradient eventually. And, if the gradient explosion is severe enough that $\lVert \mathbfit{g} \rVert$ is essentially `NaN` in software, even just a *random step* tends to work well.
...
$$
\left\lVert (\nabla_{\mathbfit{h}^{(t)}}L)\frac{ \partial \mathbfit{h}^{(t)} }{ \partial \mathbfit{h}^{(t-1)} }  \right\rVert \approx \lVert \nabla_{\mathbfit{h}^{(t)}}L \rVert,\forall t
$$
