...
&\leftarrow (1-\epsilon\alpha)\boldsymbol{w}-\epsilon \nabla_{\boldsymbol{w}}J(\boldsymbol{w};\boldsymbol{X},\boldsymbol{y})
\end{align*}
$$
...
> [!tip]+
> Remember MAP Bayesian inference from [[5. Machine Learning Basics#Maximum A Posteriori Estimation|section 5.6]]? $L^{1}$ regularization may be represented by MAP with a prior of an isotropic Laplace distribution over $\boldsymbol{w}\in \mathbb{R}^{n}$.
